...
function startGame(){game=new Game($("gameCanvas"),state);showScreen("gameScreen");game.start();window.game=game}
function openPanel(html){game.paused=true;$("panelContent").innerHTML=html;$("panelScreen").classList.remove("hidden")}
...
$("nameContinue").onclick=()=>{const n=$("nameInput").value.trim();if(!n)return;$("nameInput").blur();state.playerName=n;state.story.prologue=true;Save.save(state);startGame();game.toast("Добро пожаловать, "+n+"!");};
...
window.addEventListener("keydown",e=>{if(!game||game.paused)return;const k=e.key.toLowerCase();if(k==="j")game.attack(false);if(k==="k")game.attack(true);if(k==="e"){const x=game.player.x;const obj=game.level.objects.find(o=>Math.abs(o.x-x)<100);if(obj?.type==="computer")game.clickComputer();if(obj?.type==="shop")game.openShop();if(obj?.type==="door")game.publish();}if(k==="q"&&game.level===game.level)game.toast("SPECIAL: BEAT DROP")});
refreshQuests();
